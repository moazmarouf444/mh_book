...
@section('content')
    <x-admin.table filter="true" addbutton="{{route('admin.paper.size.create')}}"
                   deletebutton="{{route('admin.paper.size.deleteAll')}}">
            ...
            @foreach($papers as $paper)
                <tr class="delete_row">
                    <td class="text-center">
                        <label class="container-checkbox">
                            <input type="checkbox" class="checkSingle" id="{{$paper->id}}">
                            <span class="checkmark"></span>
                        </label>
                    </td>
                    <td>{{$paper->created_at->format('d/m/Y')}}</td>
                    <td>{{$paper->name}}</td>
                    <td>{{$paper->price . ' ' . __('dashboard.r.s')}} </td>
                    <td class="product-action">
                        <span class="action-edit text-primary"><a
                                    href="{{route('admin.paper.size.edit' , ['id' => $paper->id])}}"><i
                                        class="feather icon-edit"></i></a></span>
                        <span class="delete-row text-danger" data-url="{{url('admin/paper-size/'.$paper->id)}}"><i
