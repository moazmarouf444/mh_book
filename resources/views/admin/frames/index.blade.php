...
@section('content')
    <x-admin.table filter="true" addbutton="{{route('admin.frames.create')}}"
                   deletebutton="{{route('admin.frames.deleteAll')}}">
            ...
            @foreach($frames as $frame)
                <tr class="delete_row">
                    <td class="text-center">
                        <label class="container-checkbox">
                            <input type="checkbox" class="checkSingle" id="{{$frame->id}}">
                            <span class="checkmark"></span>
                        </label>
                    </td>
                    <td>{{$frame->created_at->format('d/m/Y')}}</td>
                    <td><img src="{{$frame->image ??''}}" width="50px" height="50px" alt=""></td>
                    <td>{{$frame->getTranslation('name','ar')}}</td>
                    <td>{{$frame->getTranslation('name','en')}}</td>
                    <td>{{$frame->price . ' ' . __('dashboard.r.s')}} </td>
                    <td class="product-action">
                        <span class="text-primary"><a href="{{route('admin.frames.show' , ['id' => $frame->id])}}"><i class="feather icon-eye"></i></a></span>

                        <span class="action-edit text-primary"><a
                                    href="{{route('admin.frames.edit' , ['id' => $frame->id])}}"><i
                                        class="feather icon-edit"></i></a></span>
                        <span class="delete-row text-danger" data-url="{{url('admin/frame/'.$frame->id)}}"><i
